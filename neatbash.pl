...
# 1.0  2019/09/04  BEZROUN   Cleaning of the code and domumentation for putting it on GitHub
#START ===================================================================================
#=== Start
   use v5.10;
#  use Modern::Perl;
      ...
      if( exists($delim{$first_word}) ){
         ...
         if( $top>0 ){
            ...
            if( $delim{$last_open} ne $first_word ){
               logme(__LINE__,'S',"Extended closing delimiter should be $delim{$last_open} and not $first_word");
            }
            $top--;
         }else{
...
sub  write_formatted_code
{
   ...
   if( $write_formatted ){
      ...
      `bash -n $fname.neat`;
      if(  $? > 0 ){
         logme(__LINE__,'E',"Checking reformatted code via bash -n produced some errors (RC=$?). The original file left intact. Reformatted file is $fname.neatbash");
      } else {
         close STDIN;
         `mv $fname.neat $fname`;
      }
...
my $title="\n\n".uc($main::SCRIPT_NAME).": BASF prettyprinter. Version $main::VERSION ($SCRIPT_MOD_DATE) DEBUG=$main::debug Date $timestamp";
